...
from services.services_google import upload_file, delete_file
from fastapi.responses import JSONResponse
from utils.util_routers import (
    get_all_drive_ids, 
    delete_drive_files
    )
...
from database.queries.lessons import (
    create_lesson, 
    delete_lesson_by_id
    )
from database.queries.sections import (
    add_section, 
...
async def create_section(
    ...
    return JSONResponse(
        content=f"Nueva seccion creada: {mtd_section}"
    )
...
async def delete_section(
    ...
    Errors:
        ...
        500: Google Drive deletion or database error
    """
    file_ids = get_file_ids_by_section_id(
        db=SessionLocal(),
...
async def delete_lesson(
    ...
    return JSONResponse(content="Lesson delete succesfully!", status_code=200)
...
async def edit_metadata(
    ...
    updated = update_course(
        db, 
        mtd.course_id, 
        mtd.model_dump(exclude_unset=True, 
        exclude={"id"})
        )
...
async def give_course(
    ...
    Entry:
        course_id: str (Form data - course ID to transfer)
    ...
    Errors:
        ...
        500: Database error
    """
    user_to_give = get_user_by_email(
        db=SessionLocal(),
        email=user_email_to_give
    )
    user_id = user_to_give["user_id"]

    save_response = save_purchase(
        db=SessionLocal(),
        user_id=user_id,
        course_id=course_id
    )
